...
token = None
try:
    r = httpx.post('http://127.0.0.1:8000/api/v1/auth/login', data={'username': 'admin', 'password': 'admin123'}, timeout=8.0)
...
# 3. Auth Me
try:
    r = httpx.get('http://127.0.0.1:8000/api/v1/auth/me', headers=auth_headers, timeout=8.0)
    user_data = r.json()
...
# 4. Cameras List (GIS)
try:
    r = httpx.get('http://127.0.0.1:8000/api/v1/cameras/gis', headers=auth_headers, timeout=8.0)
    cams = r.json()
...
try:
    r = httpx.post('http://127.0.0.1:8000/api/v1/cameras/sync-catalogue', headers={'X-API-Key': 'hackathon-local-ai-worker-key'}, timeout=12.0)
...
sdp_offer = '\r\n'.join(lines)
try:
    r = httpx.post('http://127.0.0.1:8000/api/v1/cameras/SENTINEL-cam01/whep', content=sdp_offer, headers={'Content-Type': 'application/sdp'}, timeout=12.0)
    loc = r.headers.get('location')
    check('Backend WHEP Proxy (/cameras/SENTINEL-cam01/whep)', r.status_code == 201, f'HTTP {r.status_code}, Session Location: {loc}')
    if loc:
        r_del = httpx.delete(f'http://127.0.0.1:8000{loc}', timeout=8.0)
...
# 7. Alerts API
try:
    r = httpx.get('http://127.0.0.1:8000/api/v1/alerts', headers=auth_headers, timeout=8.0)
...
# 8. Watchlists API
try:
    r = httpx.get('http://127.0.0.1:8000/api/v1/watchlists', headers=auth_headers, timeout=8.0)
...
try:
    r = httpx.get('http://127.0.0.1:8000/api/v1/vehicles/GJ01AB1234/route', headers=auth_headers, timeout=8.0)
    data = r.json()
...
try:
    r = httpx.get('http://127.0.0.1:9997/v3/config/paths/list', timeout=8.0)
...
try:
    r = httpx.get('http://127.0.0.1:5173/', timeout=8.0)
...
try:
    r = httpx.get('http://127.0.0.1:5173/api/v1/health', timeout=8.0)
...
try:
    r = httpx.options('http://127.0.0.1:5173/sentinel-whep/stream/cam01/whep', timeout=8.0)
...
try:
    r = httpx.get('http://127.0.0.1:5173/sentinel-hls/cam01/index.m3u8', timeout=12.0)
