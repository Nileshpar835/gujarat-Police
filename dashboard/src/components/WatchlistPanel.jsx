...
export default function WatchlistPanel() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "0 0 20px 0" }}>
      ...
      <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border-primary)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setShowCreateWL((v) => !v)}
            style={{ fontSize: 12, padding: "5px 12px", background: "var(--bg-tertiary)", border: "1px solid var(--border-primary)", borderRadius: 4, color: "var(--text-secondary)", cursor: "pointer" }}>
            + New List
          </button>
      ...
      {/* Create Watchlist form */}
      {showCreateWL && (
        <form onSubmit={handleCreateWatchlist} style={{ margin: "10px 20px", padding: 14, background: "var(--bg-primary)", borderRadius: 8, border: "1px solid var(--border-primary)", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <input required placeholder="Name (e.g. Stolen Vehicles – Ahmedabad)" value={wlForm.name} onChange={(e) => setWlForm((p) => ({ ...p, name: e.target.value }))}
            style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-primary)", borderRadius: 4, padding: "6px 10px", fontSize: 13, color: "var(--text-primary)" }} />
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button type="button" onClick={() => setShowCreateWL(false)} style={{ fontSize: 12, padding: "5px 10px", background: "var(--bg-tertiary)", border: "1px solid var(--border-primary)", borderRadius: 4, color: "var(--text-secondary)", cursor: "pointer" }}>Cancel</button>
          </div>
        </form>
      )}
      ...
      {/* Add Entry form */}
      {showAddEntry && (
        <form onSubmit={handleAddEntry} style={{ margin: "10px 20px", padding: 14, background: "var(--bg-primary)", borderRadius: 8, border: "1px solid var(--border-primary)", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <input required placeholder="Registration Number (e.g. GJ01AB1234)" value={entryForm.registration_number} onChange={(e) => setEntryForm((p) => ({ ...p, registration_number: e.target.value }))}
            style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-primary)", borderRadius: 4, padding: "6px 10px", fontSize: 13, color: "var(--text-primary)", fontFamily: "var(--font-mono)" }} />
          <div style={{ display: "flex", gap: 8 }}>
            <input placeholder="Vehicle type" value={entryForm.vehicle_type} onChange={(e) => setEntryForm((p) => ({ ...p, vehicle_type: e.target.value }))}
              style={{ flex: 1, background: "var(--bg-secondary)", border: "1px solid var(--border-primary)", borderRadius: 4, padding: "6px 10px", fontSize: 13, color: "var(--text-primary)" }} />
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button type="button" onClick={() => setShowAddEntry(false)} style={{ fontSize: 12, padding: "5px 10px", background: "var(--bg-tertiary)", border: "1px solid var(--border-primary)", borderRadius: 4, color: "var(--text-secondary)", cursor: "pointer" }}>Cancel</button>
          </div>
        </form>
      )}
      ...
      <div style={{ padding: "10px 20px", display: "flex", gap: 8, flexWrap: "wrap" }}>
        <select value={selectedWatchlist || ""} onChange={(e) => setSelectedWatchlist(e.target.value || null)}
          style={{ flex: 1, minWidth: 160, background: "var(--bg-secondary)", border: "1px solid var(--border-primary)", borderRadius: 4, padding: "6px 10px", fontSize: 13, color: "var(--text-primary)" }}>
        ...
        <input placeholder="Search plate…" value={searchPlate} onChange={(e) => setSearchPlate(e.target.value)}
          style={{ flex: 1, minWidth: 140, background: "var(--bg-secondary)", border: "1px solid var(--border-primary)", borderRadius: 4, padding: "6px 10px", fontSize: 13, color: "var(--text-primary)", fontFamily: "var(--font-mono)" }} />
      </div>
            ...
            <tbody>
              {entries.map((e) => (
                <tr key={e.id} style={{ borderBottom: "1px solid var(--border-primary)" }}>
                  ...
                  <td style={{ padding: "8px 10px" }}>
                    <span style={{ fontSize: 11, color: e.status === "active" ? "var(--accent-green)" : "var(--text-muted)" }}>{e.status}</span>
                  </td>
                  <td style={{ padding: "8px 10px", color: "var(--text-muted)", fontSize: 11 }}>{e.created_date}</td>
                </tr>
              ))}
            </tbody>
          </table>
